...
@export var SLIDE_SPEED_IMPULSE: float = 20
...
## Get the updated sliding direction. Will slide forward if [param direction] is [member Vector3.ZERO]
func _get_slide_direction(player: Player, delta: float) -> Vector3:
	# Go forward if no direction is provided by inputs
	var input_direction: Vector2 = player.get_input_direction()
	if input_direction == Vector2.ZERO:
		return player.get_look_relative_direction(Vector3.FORWARD)
